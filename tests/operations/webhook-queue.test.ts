...
it('keeps a failed event retryable instead of losing it', async () => {
  ...
  expect(await drainWebhookEvents(10, handler)).toEqual({ processed: 0, failed: 1 });
  ...
  expect(await drainWebhookEvents(10, handler)).toEqual({ processed: 1, failed: 0 });
...
it('accumulates while the drain is suspended and resumes in order without reprocessing', async () => {
  ...
  expect(await drainWebhookEvents(2, handler)).toEqual({ processed: 2, failed: 0 });
  expect(await drainWebhookEvents(10, handler)).toEqual({ processed: 1, failed: 0 });
  expect(seen).toHaveLength(3);
  expect(new Set(seen).size).toBe(3);
  // A further drain finds nothing: processed events are never handled twice.
  expect(await drainWebhookEvents(10, handler)).toEqual({ processed: 0, failed: 0 });
  expect(seen).toHaveLength(3);
});
