...
export async function productionDemand(context: AccessContext, raw: unknown) {
  ...
  const [orders, stock, supplies] = await Promise.all([readOrdersForPeriod(input), readStockSnapshot(), adminDb.collection('supplies').get()]);
  ...
  const warnings = [...stock.warnings]; if (data.some(item => item.stockLevel === null)) warnings.push('Estoque indisponível para parte dos SKUs da demanda.');
  return result(data, stock.source === 'unavailable' ? 'firestore' : 'mixed', warnings);
}
