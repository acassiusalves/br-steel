...
const definitions: ReadToolDefinition[] = [
 ...
 { name: 'resumir_vendas', title: 'Resumo de vendas', description: 'Resume vendas reais no período e compara com período anterior de mesma duração.', capability: 'vendas:read', schema: z.object(period).strict(), run: async (ctx,input) => { const r = await summarizeSales(ctx,input); return { ...r, data: { ...r.data, salesByState: r.data.salesByState.slice(0,100) }, warnings: [...r.warnings, ...(r.data.salesByState.length > 100 ? ['Distribuição por estado limitada a 100 entradas.'] : [])] }; } },
 { name: 'consultar_estoque_produtos', title: 'Estoque de produtos', description: 'Consulta saldo real de produtos, sua origem e atualização; desconhecidos são nulos.', capability: 'estoque:read', schema: paged.extend({ sku: z.string().max(200).optional() }).strict(), run: listProductStock },
 ...
 { name: 'consultar_demanda_producao', title: 'Demanda de produção', description: 'Consulta demanda por SKU com paginação e sem dados financeiros ou clientes.', capability: 'producao:read', page: '/producao', schema: paged.extend(period).strict(), run: async (ctx,input) => { const r = await productionDemand(ctx,{ from: input.from, to: input.to }); return { ...r, ...arrayPage([...r.data].sort((a,b) => a.sku < b.sku ? -1 : a.sku > b.sku ? 1 : 0),input) }; } },
 {
