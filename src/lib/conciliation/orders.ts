// Lógica pura de conciliação (sem Firebase client SDK e sem fetch).
// Pode ser importada tanto por código client quanto server (API routes, testes).
...
import { applyConciliationCustomCalculations } from "@/lib/conciliation/calculations";
import { defaultConciliationSummaryMetricIds } from "@/lib/conciliation/summary";
import { defaultConciliationSystemStatusSettings } from "@/lib/conciliation/system-status-settings";
import type {
...
export const emptyConciliationStatusSettings: ConciliationStatusSettings = {
  statusMappings: {},
  updatedAt: null,
  updatedBy: null,
};

export const emptyConciliationAccountSettings: ConciliationAccountSettings = {
  accountMappings: {},
  updatedAt: null,
  updatedBy: null,
};

export const emptyConciliationSystemStatusSettings: ConciliationSystemStatusSettings =
  defaultConciliationSystemStatusSettings;

export const emptyConciliationSummarySettings: ConciliationSummarySettings = {
  metricIds: defaultConciliationSummaryMetricIds,
  updatedAt: null,
  updatedBy: null,
};

export const emptyConciliationDivergenceSettings: ConciliationDivergenceSettings = {
  ...defaultConciliationFinancialDivergenceRules,
  updatedAt: null,
  updatedBy: null,
};

export const emptyConciliationCalculationSettings: ConciliationCalculationSettings = {
  calculations: [],
  updatedAt: null,
  updatedBy: null,
};
...
export const getConciliationAccountId = (order: ConciliationOrder): string => {
  const storeId = order.originalOrder?.loja?.id;

  return String(storeId || order.accountName || "sem-conta");
};
...
export const applyAccountMappings = (
  orders: ConciliationOrder[],
  accountMappings: ConciliationAccountMappings
): ConciliationOrder[] =>
  orders.map((order) => {
    const accountId = getConciliationAccountId(order);
    const mappedName = accountMappings[accountId]?.trim();

    if (!mappedName || mappedName === order.accountName) return order;

    return {
      ...order,
      accountName: mappedName,
      conciliation: order.conciliation?.snapshot
        ? {
            ...order.conciliation,
            snapshot: {
              ...order.conciliation.snapshot,
              accountName: mappedName,
            },
          }
        : order.conciliation,
...
const normalizeSkuKey = (value: unknown): string =>
  String(value ?? "")
    .normalize("NFD")
    .replace(/\p{Diacritic}/gu, "")
    .replace(/[^a-zA-Z0-9]/g, "")
...
/**
 * Aplica associações de planilhas aos pedidos:
 * - associações por venda viram `sheetFields` (merge por ordem de updatedAt);
 * - associações por SKU com `unitCost` substituem o custo do produto e
 *   recalculam margem.
 */
export const applySheetAssociationsToOrders = (
  orders: ConciliationOrder[],
  associations: SheetAssociationLike[]
): ConciliationOrder[] => {
  if (associations.length === 0) return orders;

  const sorted = [...associations].sort((first, second) =>
    String(first.updatedAt ?? "").localeCompare(String(second.updatedAt ?? ""))
  );
  const fieldsBySaleId = new Map<string, Record<string, string | number | null>>();
  const unitCostBySku = new Map<string, number>();

  sorted.forEach((association) => {
    if (association.targetField === "sku") {
      const unitCost = association.dataFields?.unitCost;

      if (typeof unitCost === "number" && Number.isFinite(unitCost)) {
        unitCostBySku.set(normalizeSkuKey(association.orderKey), unitCost);
      }
      return;
    }

    if (!association.saleId) return;

    const existing = fieldsBySaleId.get(association.saleId) ?? {};

    fieldsBySaleId.set(association.saleId, { ...existing, ...association.dataFields });
  });

  if (fieldsBySaleId.size === 0 && unitCostBySku.size === 0) return orders;

  return orders.map((order) => {
    const sheetFields = fieldsBySaleId.get(order.id) ?? null;

    let productCost = order.productCost;
    let costFromSheet = false;

    if (unitCostBySku.size > 0 && order.items.length > 0) {
      let total = 0;
      let matchedItems = 0;

      order.items.forEach((item) => {
        const unitCost = unitCostBySku.get(normalizeSkuKey(item.sku));
      ...
      if (matchedItems > 0) {
        // Itens sem custo na planilha mantêm proporção do custo original.
        const unmatchedOriginal = order.items.reduce((sum, item) => {
          const hasSheetCost = unitCostBySku.has(normalizeSkuKey(item.sku));

          if (hasSheetCost || order.totalQuantity === 0) return sum;

          return sum + (order.productCost / Math.max(order.totalQuantity, 1)) * item.quantity;
        }, 0);

        productCost = total + unmatchedOriginal;
        costFromSheet = true;
      }
    }

    if (!sheetFields && !costFromSheet) return order;

    const contributionMargin = order.netRevenue - productCost;
    const contributionMarginPercentage =
      order.grossRevenue > 0 ? (contributionMargin / order.grossRevenue) * 100 : 0;

    return {
      ...order,
      sheetFields: sheetFields ?? order.sheetFields,
      productCost,
      contributionMargin,
      contributionMarginPercentage,
    };
