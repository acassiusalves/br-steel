...
begin
  if nullif(event -> 'claims' ->> 'client_id', '') is not null then
    resource := nullif(event #>> '{claims,app_metadata,brsteel_mcp_resource}', '');
    if resource is null or resource = 'https://br-steel-mcp-staging.vercel.app/api/mcp' then
      resource := 'https://br-steel-mcp-staging.vercel.app/api/mcp';
    elsif resource <> 'https://br-steel.vercel.app/api/mcp' then
      raise exception using errcode = '22023', message = 'Unrecognized BR Steel MCP resource';
    end if;
    event := jsonb_set(event, '{claims,aud}', to_jsonb(resource));
  end if;
