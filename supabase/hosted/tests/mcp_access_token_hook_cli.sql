-- Project: mlumbvxpaqfzpdjnvzxc. Run after the hosted migration.
-- One prepared statement for supabase db query through the Management API.
-- All checks run as the current caller (normally the function owner).
-- No role switch, grants, persistent writes, or explicit transaction commands.
-- ACL introspection checks Auth permissions; it does NOT prove invocation as
-- supabase_auth_admin. Prove that through real Auth token issuance after activation.
-- Assertion bodies match the privileged direct test; keep both files in sync.
do $verify$
begin
declare
...
begin
  ...
  raise notice 'PASS: audience and catalog ACL assertions as caller %; real Auth invocation remains a separate check', current_user;
end;
$verify$;
