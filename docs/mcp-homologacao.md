...
Publicação da correção: `dpl_3K3Xt92mbCFYSdNUsiPBRswE5kqk`, estado `READY`, no mesmo domínio canônico. Build remoto concluído; 145 testes em 24 arquivos aprovados, typecheck com os mesmos 25 diagnósticos preexistentes e nenhuma ocorrência nova. Revisão independente sem bloqueios. Login e discovery respondem 200; MCP sem token e os três crons legados respondem 401. Metadados confirmam crons vazios e desabilitados; o manifesto original do checkout foi restaurado. [Evidência da correção](evidence/mcp-database-reads-fix-2026-09-11.json). O reteste autenticado no Claude após essa publicação foi confirmado conforme o registro abaixo; nenhuma senha, concessão ou configuração OAuth do piloto foi alterada.


## Reteste do Claude após a correção

Verificação em 12/09/2026 às 00:14 UTC: o usuário apresentou respostas que identificam corretamente leituras exclusivas do banco e ausência de registros. A auditoria confirmou duas rodadas posteriores à publicação, com `resumir_vendas`, `consultar_estoque_produtos` e `consultar_demanda_producao` em `success`. A rodada mais recente ocorreu entre 00:13:24 e 00:13:26 UTC; a conexão Claude continua ativa. [Evidência do reteste](evidence/mcp-claude-database-retest-2026-09-12.json).

As coleções de vendas, estoque, insumos e lotes seguem vazias no Firestore de homologação. Ampliar o período não encontrará dados nesse banco vazio; a situação não exige investigar o Bling. Esse aceite confirma OAuth e execução das três leituras pelo Claude sobre a base vazia. Não equivale a validar dados operacionais conhecidos, revogação/reconexão pelo cliente hospedado ou escrita. A próxima implantação com dados reais precisa usar a base operacional e suas permissões, conservando inicialmente o catálogo de leitura. Nenhuma configuração de produção foi alterada neste reteste.
