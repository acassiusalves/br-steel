...
Realtime, PostgREST, Storage, Studio, SMTP local, Edge Runtime, Analytics, Vector e pooler estão desativados/excluídos. O namespace privado contém apenas a função `private.mcp_access_token_hook(jsonb)`, `SECURITY INVOKER`, com `search_path=''`. Somente `supabase_auth_admin`, além do proprietário, recebe execução. `anon`, `authenticated` e `service_role` não podem executá-la; não há tabelas em `public`/`private`.
